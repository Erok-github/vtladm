...
# Usage:
#   sudo sh install.sh              # build + install (writes defaults to vtl.conf)
...
if [ "$ENABLE_SYSTEMD" -eq 1 ]; then
  ...
  if [ "${_VTL_REBOOT_PENDING:-0}" -eq 1 ]; then
    ...
    if [ -n "$_kspec" ] && [ -f "$_vtl_conf" ]; then
      if ! grep -qE '^[[:space:]]*vtl_instances[[:space:]]*=[[:space:]]*[^[:space:]]' "$_vtl_conf" 2>/dev/null; then
        if grep -qE '^[[:space:]]*#[[:space:]]*vtl_instances[[:space:]]*=' "$_vtl_conf" 2>/dev/null; then
          sed -i "s|^[[:space:]]*#[[:space:]]*vtl_instances=.*|vtl_instances=${_kspec}|" "$_vtl_conf"
        else
          echo "vtl_instances=${_kspec}" >> "$_vtl_conf"
        fi
        echo "  synced vtl_instances=${_kspec} into vtl.conf (before reboot)"
      fi
    fi
  ...
  if [ "$_kernel_ok" -eq 1 ]; then
    ...
    if [ -n "$_kspec" ] && [ -f "$_vtl_conf" ]; then
      if ! grep -qE '^[[:space:]]*vtl_instances[[:space:]]*=[[:space:]]*[^[:space:]]' "$_vtl_conf" 2>/dev/null; then
        ...
        echo "  synced vtl_instances=${_kspec} into vtl.conf"
      fi
    fi
    ...
    if ! systemctl restart vtladm-web.service 2>/dev/null; then
      ...
      _wp=8765
      if [ -f "$_vtl_conf" ]; then
        _wp=$(grep -E '^[[:space:]]*web_port[[:space:]]*=' "$_vtl_conf" 2>/dev/null | tail -1 \
