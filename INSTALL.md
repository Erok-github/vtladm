...
| `/opt/vtladm/var/vtl.conf` | 模块参数、Web 绑定地址 |
...
sudo sed -i 's/\r$//' install.sh packaging/sbin/* packaging/scripts/*.sh /opt/vtladm/var/vtl.conf
sudo sh install.sh --enable
...
| `Unknown parameter` 且参数名带奇怪后缀 | `/opt/vtladm/var/vtl.conf` 含 CRLF：`sed -i 's/\r$//' /opt/vtladm/var/vtl.conf` |
...
默认用户 **`admin`**，初始密码在首次安装时随机生成并打印到终端（`vtladm serve` 输出的 stderr）。须正确填写**算术验证码**（错误时提示「验证码错误或已过期」）。
