...
static int vtl_handle_read_block_limits(struct scsi_cmnd *cmd, struct vtl_drive *drv)
{
    ...
    buffer[0] = (VTL_MAX_BLOCK_SIZE >> 16) & 0xff;
    buffer[1] = (VTL_MAX_BLOCK_SIZE >> 8) & 0xff;
    buffer[2] = VTL_MAX_BLOCK_SIZE & 0xff;
    buffer[3] = (VTL_MIN_BLOCK_SIZE >> 8) & 0xff;
    buffer[4] = (VTL_MIN_BLOCK_SIZE >> 0) & 0xff;
...
static int vtl_handle_mode_sense(struct scsi_cmnd *cmd, struct vtl_host *vhost)
{
    ...
    if (lun == 0) {
        ...
        wp = (drv && drv->loaded_tape && drv->loaded_tape->write_protected) ? 0x80 : 0;
...
static int vtl_handle_write(struct scsi_cmnd *cmd, struct vtl_drive *drv, u8 op)
{
    ...
    if (drv->loaded_tape->write_protected) {
    ...
    ret = vtl_tape_write(drv, buffer, blocks * block_len);
    vtl_xfer_buf_free(buffer);
...
static int vtl_handle_write_filemarks(struct scsi_cmnd *cmd, struct vtl_drive *drv)
{
    ...
    count = (cdb[7] << 8) | cdb[8];
    vtl_tape_write_filemarks(drv, count);
    return SAM_STAT_GOOD;
}
...
static int vtl_handle_log_sense(struct scsi_cmnd *cmd, struct vtl_drive *drv)
{
    ...
    if (page == 0x00) {
        ...
        buf[2] = 0;
        buf[3] = 16;
        vtl_put_be64(drv->loaded_tape->meta.log_bytes_read, &buf[4]);
        vtl_put_be64(drv->loaded_tape->meta.log_bytes_written, &buf[12]);
        out_len = 20;
    } else {
        vtl_xfer_buf_free(buf);
...
static int vtl_handle_read_position(struct scsi_cmnd *cmd, struct vtl_drive *drv)
{
    ...
    if (drv->loaded_tape) {
        ...
        if (drv->at_filemark)
            buf[1] |= 0x20;
        vtl_put_be64((u64)drv->loaded_tape->position, &buf[4]);
    } else {
        buf[1] |= 0x10;
    }
