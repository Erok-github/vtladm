...
fn import_offline_tape_to_online_slot(
    ...
    let (tape_id, shelf_id, current_slot, image_path): (i64, Option<i64>, Option<i32>, String) = conn
        .query_row(
    ...
    for (tape_name, slot, from_offline) in pairs {
        if *from_offline {
            ...
            if robot_sync::robot_ioctl_enabled() {
                let bc = robot_sync::tape_barcode_for_name(
                    &conn,
                    online_lib_id,
                    tape_name.as_str(),
                );
...
fn plan_b_idle_host_segment() -> (i32, i32) {
    (VTL_KERNEL_MAX_DRIVES_PER_LIB, VTL_KERNEL_MAX_DATA_SLOTS_PER_LIB)
}
...
pub(crate) fn online_library_export_index(
    ...
    let mut stmt = conn.prepare(
        "SELECT id FROM vtl_libraries WHERE name NOT IN (?1, ?2) ORDER BY id ASC",
    )?;
...
fn refuse_ioctl_if_vtl_holders_busy() -> Option<String> {
    if std::env::var("VTL_FORCE_GEOM_IOCTL")
        .ok()
        .as_deref()
        == Some("1")
    {
...
fn run_vtl_kernelctl_reload() -> Result<(), String> {
    let ctl = find_vtl_kernelctl_bin()
        .ok_or_else(|| "vtl-kernelctl not found under /opt/vtladm/sbin or /usr/local/sbin".to_string())?;
...
pub(crate) fn maybe_reload_kernel_vtl_after_db_change() -> KernelGeomSync {
    ...
    if std::env::var("VTL_SKIP_SPEC_CACHE").ok().as_deref() != Some("1")
        && std::env::var("VTL_NO_SCSI_RESCAN_ON_UNCHANGED_SPEC").ok().as_deref() != Some("1")
    {
        ...
        match ioctl_result {
            Ok(()) => {
                ...
                return KernelGeomSync::status("ioctl_ok", spec)
                    .with_post_geom_rescan("ioctl_ok");
            }
            Err(e) => {
    ...
    let script = match cfg_now.kernel_vtl_reload_script {
        ...
        None => {
            ...
            return KernelGeomSync::status("no_fallback", "ioctl failed or disabled; no kernel_vtl_reload_script");
        }
    };
...
pub(crate) fn write_initial_vtl_conf(params: &InitialVtlConfParams) -> Result<(), VtlError> {
    ...
    fs::write(
        primary_vtl_conf_path(),
        format_initial_vtl_conf(params),
    )?;
    Ok(())
}
...
fn log_message(msg: &str) {
    ...
    if let Err(e) = try_append_log_line_in(
        &config.log_dir,
        "vtladm.log",
        &line,
...
enum Commands {
    ...
    KernelSpec {
        #[arg(long, help = "Plan B: 8 libraries × 8 drives × 256 slots insmod topology")]
        insmod_max: bool,
    },
...
fn validate_tape_name(name: &str) -> Result<(), VtlError> {
    if name.is_empty() {
        return Err(VtlError::InvalidTapeName("Tape name cannot be empty".to_string()));
    }
    if name.len() > 255 {
        return Err(VtlError::InvalidTapeName("Tape name too long (max 255 characters)".to_string()));
    }
    let invalid_chars = ['/', '\\', ':', '*', '?', '"', '<', '>', '|', '\0'];
    for c in name.chars() {
        if invalid_chars.contains(&c) {
            return Err(VtlError::InvalidTapeName(format!("Invalid character in tape name: '{}'", c)));
        }
    }
    Ok(())
}
...
fn create_tape(name: &str, size: u64, shelf_name: Option<&str>) -> Result<(), VtlError> {
    ...
    match fs::create_dir_all(&lib_dir) {
        ...
        Err(e) if e.kind() == std::io::ErrorKind::PermissionDenied => {
            log_error("create_tape", &format!("Permission denied: {}", lib_dir.display()));
...
fn check_quota(size: u64) -> Result<(), VtlError> {
    ...
    let max_total_size = lib_config_get(&conn, library_id, "quota_max_total_size")
        .or_else(|| {
            conn.query_row(
...
fn batch_create_tapes(count: i32, size: u64, prefix: &str, tags: &[String]) -> Result<(), VtlError> {
    log_message(&format!("Batch creating {} tapes with prefix '{}'", count, prefix));
...
fn batch_import_tapes(directory: &str, start_slot: i32) -> Result<(), VtlError> {
    ...
    if !dir_path.exists() || !dir_path.is_dir() {
        return Err(VtlError::InvalidParameter(format!("Directory not found: {}", directory)));
    }
...
fn quota_check() -> Result<(), VtlError> {
    ...
    if let Some(max_total_size_str) = max_total_size {
        ...
        if current_total > max_total_size_bytes {
            ...
            println!("  Current: {}, Max: {}", format_size(current_total), format_size(max_total_size_bytes));
            exceeded = true;
        }
    }
...
fn tag_remove(tape_name: &str, tags: &[String]) -> Result<(), VtlError> {
    log_message(&format!("Removing tags {:?} from tape '{}'", tags, tape_name));
    ...
    for tag_name in tags {
        if let Ok(tag_id) = tx.query_row("SELECT id FROM tags WHERE name = ?1", params![tag_name], |row| row.get::<usize, i64>(0)) {
            tx.execute(
    ...
    log_message(&format!("Successfully removed tags from tape '{}'", tape_name));
...
fn tag_delete(tag_name: &str) -> Result<(), VtlError> {
    ...
    let tag_id: i64 = conn
        .query_row("SELECT id FROM tags WHERE name = ?1", params![tag_name], |row| row.get::<usize, i64>(0))
...
pub(crate) fn delete_tape_in_library(library: &str, name: &str) -> Result<(), VtlError> {
    ...
    tx.execute(
        "DELETE FROM tape_tags WHERE tape_id = ?1",
        params![tape_id],
    )?;
    ...
    if let Err(e) = fs::remove_file(&image_path) {
        log_error(
            ...
            &format!(
                "DB committed but failed to remove tape image {}: {}",
                image_path, e
            ),
        );
        return Err(VtlError::from(e));
...
pub(crate) fn delete_shelf_in_library(library: &str, shelf_name: &str) -> Result<(), VtlError> {
    let conn = init_db()?;
    let library_id = resolve_library_id(&conn, library)?;
    let (shelf_id, is_default): (i64, i64) = conn.query_row(
...
pub(crate) fn delete_named_library(name: &str) -> Result<(Vec<String>, KernelGeomSync), VtlError> {
    ...
    if online_count <= 1 {
        ...
        let sole_legacy_default =
            name == LEGACY_DEFAULT_LIBRARY_NAME && tape_count == 0;
    ...
    for tape in tapes {
        ...
        let (location, note) = if let Some(ref kmap) = kernel_locs {
            if let Some(loc) = kmap.get(&name) {
                ...
                let locs = match loc {
                    MediumLocation::Drive(d) => (format!("drive{}", d), "In drive (kernel)".to_string()),
                    MediumLocation::DataSlot(s) => (format!("slot{}", s), "In robot (kernel)".to_string()),
                    MediumLocation::MailSlot(m) => {
                        (format!("mail{}", m - MAILSLOT_OFFSET), "In IE (kernel)".to_string())
                    }
                };
                locs
            } else if in_drive > 0 {
...
pub(crate) fn load_tape_in_library(
    library: &str,
    slot: i32,
    drive: i32,
) -> Result<(), VtlError> {
...
fn load_tape(slot: i32, drive: i32) -> Result<(), VtlError> {
    log_message(&format!("Loading tape from slot {} to drive {}", slot, drive));
...
fn eject_tape(slot: i32) -> Result<i32, VtlError> {
    ...
    if robot_sync::robot_ioctl_enabled() {
        ...
        if let Err(e) =
            robot_sync::kernel_move_medium(&conn, library_id, slot, dst)
        {
...
fn snapshot_tape(tape: &str, snapshot: &str) -> Result<(), VtlError> {
    log_message(&format!("Creating snapshot '{}' for tape '{}'", snapshot, tape));
    ...
    fs::copy(&image_path, &snapshot_path)?;

    log_message(&format!(
        "Successfully created snapshot '{}' for tape '{}' at {}",
        snapshot, tape, snapshot_path.display()
    ));
...
fn import_tape(path: &str, slot: i32) -> Result<(), VtlError> {
    ...
    let tape_name = file_name.strip_suffix(".vtltape").unwrap_or(&file_name).to_string();
...
fn export_tape(slot: i32, output: &str, checksum: bool) -> Result<(), VtlError> {
    log_message(&format!("Exporting tape from slot {} to '{}'", slot, output));
    ...
    println!("Exported tape '{}' from slot {} to {}", tape_name, slot, output);
...
pub(crate) fn create_named_library(name: &str, drives: i32, slots: i32) -> Result<KernelGeomSync, VtlError> {
    if name.is_empty() {
        return Err(VtlError::InvalidParameter(
            "Library name cannot be empty".to_string(),
        ));
    ...
    } else if matches!(geom.kernel_geom.as_str(), "ioctl_ok" | "reload_ok" | "script_ok") {
        println!(
...
fn list_shelves() -> Result<(), VtlError> {
    ...
    for row in stmt.query_map(params![library_id], |r| {
        ...
        let tag = if is_def != 0 { " [default / 未使用]" } else { "" };
...
fn create_shelf(shelf_name: &str) -> Result<(), VtlError> {
    ...
    println!("Created shelf '{}' in library '{}'", shelf_name, current_library_name());
    Ok(())
}
...
fn list_shelf_tapes(shelf_filter: Option<&str>) -> Result<(), VtlError> {
    ...
    if let Some(sname) = shelf_filter {
        ...
        let mut sh = conn.prepare(
            "SELECT id, name FROM shelves WHERE library_id = ?1 ORDER BY name",
        )?;
...
fn main() -> Result<(), VtlError> {
    ...
    match &cli.command {
        Commands::CreateTape { name, size, shelf, tags } => {
        ...
        Commands::BatchCreate { count, size, prefix, tags } => {
            let size_bytes = parse_size(size).unwrap_or_else(|e| {
                log_error("main", &e.to_string());
                eprintln!("Error: {}", e);
                process::exit(1);
        ...
        Commands::BatchImport { directory, start_slot } => {
            batch_import_tapes(directory, *start_slot)?;
        }
        Commands::Quota { subcommand } => match subcommand {
            QuotaSubcommand::Set { max_total_size, max_tapes } => {
        ...
        Commands::Search { name, tag, min_size, max_size, free_space } => {
            search_tapes(name.clone(), tag.clone(), min_size.clone(), max_size.clone(), *free_space)?;
        }
        ...
        Commands::KernelAlign { quiet } => {
            ...
            match geom.kernel_geom.as_str() {
                ...
                "lio_exported" | "holders_busy" | "script_refused_holders"
...
mod tests {
    ...
    fn test_default_paths_under_install_prefix() {
        for p in [
            ...
            assert!(
                ...
                "{p} not under {VTL_INSTALL_PREFIX}"
            );
        }
    ...
    fn test_build_vtl_instances_kernel_spec_padded() {
        ...
        assert_eq!(
            spec,
            "2x10,8x256,8x256,8x256,8x256,8x256,8x256,8x256"
        );
        cleanup_temp_vtl(&dir);
    }
    ...
    fn test_integration_workflow() {
        ...
        let p = dir.join("tapes").join("default").join("test_tape_1.vtltape");
    ...
    fn test_snapshot_functionality() {
        ...
        let snapshot_path = dir.join("tapes").join("default").join("snap_test_snap_001.vtltape");
    ...
    fn test_export_functionality() {
        ...
        let p = dir.join("tapes").join("default").join("export_test.vtltape");
    ...
    fn test_multiple_tapes_inventory() {
        ...
        for i in 0..4 {
            ...
            let p = dir.join("tapes").join("default").join(format!("multi_tape_{}.vtltape", i));
    ...
    fn test_batch_import_tapes() {
        ...
        fs::copy(
            dir.join("tapes").join("default").join("import_tape_1.vtltape"),
            import_dir.join("import_tape_1.vtltape"),
        )
        .unwrap();
        fs::copy(
            dir.join("tapes").join("default").join("import_tape_2.vtltape"),
    ...
    fn test_search_functionality() {
        ...
        let result = search_tapes(None, None, Some("5M".to_string()), Some("25M".to_string()), None);
        assert!(result.is_ok());
    ...
    fn test_migrate_tapes_between_shelves_ok() {
        ...
        let _ = migrate_tapes_between_shelves(
            "default",
            "sA",
            "sB",
            &[String::from("tmove")],
    ...
    fn test_assign_offline_tape_into_online_library_slot() {
        ...
        let _ = assign_tapes_to_slots_batch(
            "default",
            &[(String::from("off_in"), 0, true)],
        )
        .unwrap();
        let conn = init_db().unwrap();
        let lid = resolve_library_id(&conn, "default").unwrap();
