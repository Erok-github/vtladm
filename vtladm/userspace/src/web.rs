...
async fn api_fabric() -> impl IntoResponse {
    ...
    let body = json!({
        ...
        "fc_wwpn": c.fc_wwpn,
        "db_path": c.db_path.to_string_lossy(),
        "tape_dir": c.tape_dir.to_string_lossy(),
        "log_dir": c.log_dir.to_string_lossy(),
        "kernel_vtl_reload_script": c.kernel_vtl_reload_script.as_ref().map(|p| p.to_string_lossy()),
        "kernel_reload_on_db_change": c.kernel_reload_on_db_change,
        "kernel_geom_prefer_ioctl": c.kernel_geom_prefer_ioctl,
        "vtl_ko": c.vtl_ko.as_ref().map(|p| p.to_string_lossy()),
...
fn json_cookie_response(status: StatusCode, body: serde_json::Value, set_cookie: Option<&str>) -> Response {
...
async fn api_login(
    ...
    match st.login(username, password) {
        ...
        Err(super::web_auth::LoginFail::BadCredentials) => json_cookie_response(
            StatusCode::UNAUTHORIZED,
            json!({
                "error": "用户名或密码错误",
                "hint": format!(
...
async fn api_logout(
    ...
    json_cookie_response(
        StatusCode::OK,
        json!({ "ok": true }),
        Some("vtl_session=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0"),
    )
}
...
async fn api_change_password(
    ...
    match st.change_password(&body.old_password, &body.new_password) {
        Ok(()) => json_cookie_response(StatusCode::OK, json!({ "ok": true }), None),
        Err(e) => json_cookie_response(
            StatusCode::BAD_REQUEST,
            json!({ "error": e }),
            None,
        ),
...
async fn api_manage_tape_create_auto_batch(
    ...
    let res = tokio::task::spawn_blocking(move || {
        let size = super::parse_size(&size_s).map_err(|e| e.to_string())?;
        super::create_auto_named_tapes_batch(&lib, shelf.as_deref(), count, size).map_err(|e| e.to_string())
    })
    .await
    .map_err(|e| e.to_string())
    .and_then(|r| r);
...
fn validate_iqn(s: &str) -> Result<(), String> {
    ...
    if s.contains('_') {
        return Err(
            "IQN 不得含下划线 _（新版 LIO/rtslib 会报 WWN not valid；冒号后请用连字符 -，或点 .）".into(),
        );
    }
    if s.chars().any(|c| {
        c.is_control()
...
fn validate_iscsi_portal_host(host: &str) -> Result<(), String> {
    ...
    if t.contains(':') {
        return Err("门户监听地址不可含冒号（不支持 IPv6 字面量；请用 IPv4 或可解析主机名）".into());
    }
    if t.chars().any(|c| {
        c.is_whitespace()
            || c.is_control()
...
fn iscsi_library_exec_hint(dry_run: bool, ok: bool, stderr: &str, is_export: bool) -> Option<serde_json::Value> {
...
fn pick_vtl_host_for_scan(
    ...
    if groups.is_empty() {
        return Err(
            "lsscsi 输出中未发现 VTL SCSI 设备（请确认已加载 vtl.ko 且已 scan）"
                .into(),
        );
    }
...
fn build_transport_scan_response(
    ...
    m.insert(
        "note".into(),
        json!(transport_scan_hint(transport)),
    );
...
"#;
    ...
    fn st_path_lookup_from_lsscsi_line() {
        const OUT: &str = "[33:0:0:1]   tape       VTL      VTL TAPE DRV 1.00  /dev/st0    /dev/sg3\n";
        assert_eq!(
...
fn ensure_tape_file_under_tape_dir(path_str: &str) -> Result<PathBuf, String> {
    ...
    let canon_root = std::fs::canonicalize(&root)
        .map_err(|e| format!("无法 canonicalize tape_dir: {}", e))?;
...
) -> Result<(String, String, Option<Vec<u32>>, Option<u32>, Option<String>), String> {
...
async fn api_manage_iscsi_library_export_defaults(
    ...
    let lib_c = lib.clone();
    let res = tokio::task::spawn_blocking(move || -> Result<(i64, Option<super::iscsi_export::IscsiLibraryExportRecord>), String> {
        let conn = super::init_db().map_err(|e| e.to_string())?;
    ...
    let (iqn, export_id, portal_ip, portal_port, default_lun_map, changer_sg, drive_sg, has_saved_export, exported_at, saved_drive_mismatch) =
        if !q.regenerate {
    ...
    let export_blocked_reason: serde_json::Value = if can_export {
        ...
        json!("当前库驱动器数为 0，无法执行 library-export（内核 vtl 须至少暴露一台磁带机 /dev/sg）")
    };
...
async fn api_manage_transport_scan_sg(
    ...
    let lib_name = match q.library.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        None => {
            return (
                StatusCode::BAD_REQUEST,
                Json(json!({
...
async fn api_manage_iscsi_library_export(
    ...
    let drv_check = tokio::task::spawn_blocking(move || -> Result<(), String> {
        let conn = super::init_db().map_err(|e| e.to_string())?;
        let library_id = super::resolve_library_id(&conn, &lib_for_drv).map_err(|e| e.to_string())?;
        let drive_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM drives WHERE library_id = ?1",
                rusqlite::params![library_id],
        ...
        move || -> Result<(std::process::Output, bool), String> {
            ...
            if ok && !dry_run && !blocks_db {
                ...
                let library_id = super::resolve_library_id(&conn, &library_for_save)
                    .map_err(|e| e.to_string())?;
                let lun_map = lun_map_for_save.unwrap_or_else(|| {
                    (0..=drive_sg.len() as u32).collect()
                });
...
async fn api_manage_iscsi_library_unexport(
    ...
    match res {
        Ok(Ok((out, lib_cleared))) => {
            ...
            (
                status,
                Json(serde_json::Value::Object(j)),
            )
                .into_response()
        }
        Ok(Err(e)) => (
...
pub(crate) fn run_web_ui(host: &str, port: u16) -> Result<(), super::VtlError> {
    let addr: SocketAddr = format!("{}:{}", host, port)
        .parse()
        .map_err(|_| {
            super::VtlError::InvalidParameter(format!(
                "Invalid web bind address {}:{}",
                host, port
            ))
        })?;
    ...
    eprintln!(
        "VTL web UI: http://{}:{}  （须登录后访问页面与 API；默认用户 {}，初始密码见登录页；认证文件 {})",
        addr.ip(),
        addr.port(),
        super::web_auth::DEFAULT_WEB_USER,
    ...
    eprintln!(
        "若无法登录：在服务器执行 vtladm reset-web-auth 后 systemctl restart vtladm-web"
    );
    eprintln!(
    ...
    rt.block_on(async move {
        ...
        axum::serve(listener, app)
            .await
            .map_err(|e| super::VtlError::IoError(std::io::Error::new(std::io::ErrorKind::Other, e)))
    })
}
...
<h1>Web 登录</h1>
<p class="login-hint">默认用户 <code>admin</code>，初始密码 <code>4rfVBNji9</code>（请登录后在管理页尽快修改）。首次运行会在日志目录生成 <code>web_admin.json</code>（bcrypt 哈希）。</p>
<form id="f">
...
mod iscsi_validate_tests {
    use super::{
        iscsi_library_exec_hint, resolve_iscsi_unexport_params, validate_iqn,
        validate_iscsi_portal_host, validate_lun_map_consecutive_from_zero,
        validate_lun_map_values, IscsiLibraryUnexportBody,
    };
...
mod web_html_tests {
    ...
    fn test_extract_vtl_session_cookie_accepts_prefix_before_name() {
        let res = axum::response::Response::builder()
            .header(header::SET_COOKIE, "Path=/; vtl_session=testtoken123; HttpOnly")
            .body(Body::empty())
            .expect("build synthetic login response");
        assert_eq!(
            extract_vtl_session_cookie(&res),
            "vtl_session=testtoken123"
        );
    }
    ...
    async fn web_login_cookie(app: Router<()>) -> (String, Router<()>) {
        ...
        let cap_j: serde_json::Value = serde_json::from_slice(&cap_body)
            .expect("GET /api/captcha JSON body");
    ...
    fn test_web_html_login_shell_card_no_boot() {
        ...
        assert!(!h.contains("normPath"), "login must not embed web_boot (normPath)");
        assert!(
