...
pub(crate) fn try_scsi_rescan_after_geom_change(context: &str) -> &'static str {
    if std::env::var("VTL_NO_SCSI_RESCAN_AFTER_GEOM").ok().as_deref() == Some("1") {
        return "skipped";
    }
...
pub(crate) fn scsi_rescan_vtl_hosts_under(sys_scsi_host: &Path, stagger_ms: u64) -> Result<(), String> {
    use std::thread;
    ...
    scan_targets.sort_by_key(|p| {
        p.parent()
            .and_then(scsi_host_dir_index)
            .unwrap_or(u32::MAX)
    });
    ...
    for (i, scan_path) in scan_targets.iter().enumerate() {
        std::fs::write(scan_path, SCAN_LINE).map_err(|e| {
            format!(
                "write {} (need root?): {}",
                scan_path.display(),
                e
...
pub(crate) fn scsi_rescan_scsi_host(host: u32) -> Result<(), String> {
    ...
    std::fs::write(&scan_path, SCAN_LINE).map_err(|e| {
        format!("write {} (need root?): {}", scan_path.display(), e)
    })
}
...
mod tests {
    ...
    fn rescan_writes_only_vtl_hosts_deterministic_order() {
        ...
        assert_eq!(fs::read_to_string(scsi.join("host_other/scan")).unwrap(), "");
        let _ = fs::remove_dir_all(&root);
    }
