...
pub(crate) fn save_iscsi_library_export(
    ...
    let drive_sg_json =
        serde_json::to_string(&record.drive_sg).map_err(|e| VtlError::InvalidParameter(e.to_string()))?;
    let lun_map_json =
        serde_json::to_string(&record.lun_map).map_err(|e| VtlError::InvalidParameter(e.to_string()))?;
    conn.execute(
        r#"
...
pub(crate) fn load_iscsi_library_export(
    ...
    match row {
        Ok((iqn, export_id, changer_sg, drive_sg_json, lun_map_json, portal_ip, portal_port, exported_at)) => {
...
pub(crate) fn delete_iscsi_library_export(conn: &Connection, library_id: i64) -> Result<(), VtlError> {
    conn.execute(
...
pub(crate) fn list_iscsi_library_exports(
    ...
    let mut out = Vec::new();
    for row in rows {
        let (name, iqn, export_id, changer_sg, drive_sg_json, lun_map_json, portal_ip, portal_port, exported_at) =
            row?;
        let drive_sg: Vec<String> = serde_json::from_str(&drive_sg_json)
            .map_err(|e| VtlError::InvalidParameter(format!("drive_sg_json: {}", e)))?;
        let lun_map: Vec<u32> = serde_json::from_str(&lun_map_json)
