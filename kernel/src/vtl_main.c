...
static int __init vtl_init(void)
{
    ...
    vtl_restore_all_changer_states();
...
static void __exit vtl_exit(void)
{
    ...
    /*
     * Set the unloading flag BEFORE tearing down any user-facing interfaces
     * so ioctl / sysfs handlers can reject new requests immediately.  The
     * memory barrier pairs with the read in vtl_module_is_unloading() and
     * ensures in-flight handlers on other CPUs observe the flag before we
     * start freeing data structures.
     */
    atomic_set(&vtl_module_unloading, 1);
    smp_wmb();

    /*
     * Tear down user-facing interfaces first; in-flight ioctls still run.
     * synchronize_rcu() after each teardown ensures any thread that entered
     * an ioctl / sysfs handler before we set vtl_module_unloading has exited
     * before we free the module-static fops, attrs, and kobj below.
     */
    vtl_misc_exit();
    synchronize_rcu();
    if (vtl_geom_wq)
