...
use crate::{get_config, log_error, log_message, OFFLINE_LIBRARY_NAME, VtlError};
...
fn ioctl_vtl<T>(cmd: libc::c_ulong, req: &mut T) -> io::Result<()> {
    ...
    let r = unsafe {
        libc::ioctl(
            f.as_raw_fd(),
            cmd,
            req as *mut T as *mut libc::c_void,
        )
    };
    if r < 0 {
...
pub(crate) fn online_library_ids_for_kernel_sync(
    conn: &Connection,
) -> Result<Vec<i64>, VtlError> {
    ...
    let ids: Vec<i64> = stmt
        .query_map(
            params![
                ...
                crate::LEGACY_DEFAULT_LIBRARY_NAME
            ],
            |r| r.get(0),
        )?
        .collect::<Result<_, _>>()?;
...
pub(crate) fn ensure_inventory_complete(
    snap: &KernelInventory,
) -> Result<(), VtlError> {
    if snap.truncated {
        return Err(VtlError::InvalidParameter(format!(
...
pub(crate) fn evacuate_shelved_tapes_from_kernel(
    ...
    let mut stmt = conn.prepare(
        "SELECT name FROM tapes WHERE library_id = ?1 AND shelf_id IS NOT NULL",
    )?;
...
fn tape_barcode_map(conn: &Connection, library_id: i64) -> Result<HashMap<String, String>, VtlError> {
    let mut out = HashMap::new();
    let mut stmt = conn.prepare(
        "SELECT name, barcode FROM tapes WHERE library_id = ?1",
    )?;
    let rows = stmt.query_map(params![library_id], |r| {
...
fn changer_inventory_from_kernel(
    ...
    let mut drives = Vec::new();
    let mut dstmt = conn.prepare(
        "SELECT drive_id FROM drives WHERE library_id = ?1 ORDER BY drive_id",
    )?;
...
mod tests {
    ...
    fn ioctl_numbers_match_kernel_ioc_macro() {
        ...
        assert_eq!(
            mk(2, load_sz) as libc::c_ulong,
            super::VTL_IOCTL_LOAD_TAPE
        );
        assert_eq!(
