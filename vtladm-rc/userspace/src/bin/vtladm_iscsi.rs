...
fn run(cli: Cli) -> Result<(), String> {
    match &cli.command {
        ...
        Commands::QuickUnexport {
            iqn,
            fileio_name,
        } => cmd_quick_unexport(&cli, iqn, fileio_name),
...
fn cmd_check() -> Result<(), String> {
    ...
    println!(
        "targetcli in PATH: {}",
        if ok { "yes" } else { "no (install targetcli / python3-rtslib-fb)" }
    );
    println!(
...
fn library_export_phase2_targetcli_script(
    ...
    s.push_str(&format!("{} set attribute demo_mode_write_protect=0\n", base));
    s.push_str(&format!("{} set attribute generate_node_acls=1\n", base));
    s.push_str(&format!("{}/luns/ create /backstores/pscsi/{}\n", base, ch_bs));
...
fn cmd_library_unexport(
    ...
    let lun_numbers: Vec<u32> = if let Some(s) = lun_map {
        ...
        return Err("library-unexport: specify --drives (with sequential lun0..) or --lun-map".into());
    };
    let path_style = resolve_iscsi_shell_path_style(cli)?;
...
mod tests {
    ...
    fn test_library_export_script_order() {
        ...
        assert!(s.contains(&format!("{} set attribute demo_mode_write_protect=0\n", base)));
    ...
    fn test_library_export_script_merged_path_no_tpg1_segment() {
        ...
        assert!(s.contains(&format!(
            "{}/luns/ create /backstores/pscsi/mm1_ch\n",
            base
        )));
    }
    ...
    fn test_parse_comma_lun_map() {
        assert_eq!(
            parse_comma_lun_map("3,4,5", 3).unwrap(),
            vec![3, 4, 5]
        );
