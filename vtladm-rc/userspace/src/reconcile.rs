...
use crate::robot_sync::{
    self, MediumLocation, db_inventory, ensure_inventory_complete, evacuate_shelved_tapes_from_kernel,
    evacuate_tape_from_changer, kernel_inventory_snapshot, robot_sync_enabled,
    warn_kernel_sync_failed,
};
...
pub(crate) fn pull_library_inventory_from_kernel(
    library_id: i64,
...
pub fn reconcile_current_library(apply_db: bool, pull_kernel: bool) -> Result<ReconcileReport, VtlError> {
    let conn = init_db()?;
...
pub fn print_reconcile_report(lib_name: &str, report: &ReconcileReport) {
    if report.inventory_truncated {
        eprintln!(
            "Warning: kernel GET_INVENTORY was truncated; results may be incomplete."
        );
    }
...
fn drifts_safe_for_pull(drifts: &[DriftItem]) -> bool {
    drifts.iter().all(|d| d.db.is_some() && d.kernel.is_some() && d.db != d.kernel)
}
...
fn auto_align_library_inner(library_id: i64, allow_pull: bool) -> Result<AutoAlignReport, VtlError> {
    if !robot_sync_enabled() {
    ...
    let want_pull = allow_pull
        && auto_reconcile_pull_enabled()
        && drifts_safe_for_pull(&drifts);
    ...
    if want_pull {
        ...
        report.pull_updates =
            apply_kernel_to_db(&mut conn, library_id, &fresh.locations)?;
        drifts.clear();
        log_message(&format!(
